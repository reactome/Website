...
!function($) {
    ...
    function updateMediaUrl(row, options, repeatable) {
        $(row).find(".field-media-wrapper").add(row).each(function() {
            ...
            var dataUrl = $(this).data("url") || $(this).attr("url") || "", $linkBtn = $(this).find('a[href*="index.php?option=com_media"].modal.btn'), params = parseUrl(dataUrl = $linkBtn.length && !dataUrl ? $linkBtn.attr("href") || "" : dataUrl), dataUrl = "images", plugin = params.plugin || "", $inp = (params.mediatype ? dataUrl = params.mediatype : "files" == params.view && (dataUrl = "files"), 
            getBasePath($inp) + "index.php?option=com_jce&task=mediafield.display&plugin=" + plugin + "&fieldid=" + id + "&mediatype=" + dataUrl);
            options.context && (params.context = options.context);
            $.each([ "option", "task", "plugin", "fieldid", "mediatype", "element" ], function(i, key) {
                delete params[key];
