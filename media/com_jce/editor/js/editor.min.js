        ...
        preinit: function(elements) {
            ...
            try {
                elements = elements || DOM.select(".wf-editor"), each(elements, function(elm) {
                    if (!(elm = DOM.get(elm))) throw new Error("Element not found");
                    var editorId = function(elm) {
                        var id = elm.id;
                        return id || (id = (id = elm.name) && !DOM.get(id) ? elm.name : DOM.uniqueId(), 
                        elm.setAttribute("id", id)), id;
                    }(elm);
                    if (tinymce.get(editorId)) throw new Error("Editor already exists");
                    DOM.show(elm), "advanced" == settings.theme && self.createInstance(elm), 
