...
!function() {
    ...
    var JoomlaEditor = {
        ...
        init: function(settings) {
            if (!(settings = settings || function(key) {
                ...
                if ((scripts = Joomla.getOptions ? Joomla.getOptions("plg_editor_jce", {}) : Joomla.optionsStorage.plg_editor_jce || {}) && scripts[key]) return scripts = scripts[key], 
                scripts = Array.isArray(scripts) ? scripts[0] : scripts;
            }("editor"))) throw new Error("Unable to initialize editor. No settings found");
        ...
        load: function() {
            ...
            tinyMCE.onAddEditor.add(function(mgr, ed) {
                ...
                JoomlaEditor.register(ed)), self.settings.compress.css && ed.onPreInit.add(function() {
                    ed.dom.loadCSS(self.settings.site_url + "index.php?option=com_jce&task=editor.pack&type=css&slot=content");
                }), self.canToggle() && self.createToggle(el), self.setBookmark(ed), 
        ...
        createTabs: function(ed) {
            var self = this, settings = this.settings, el = ed.getElement();
        ...
        convertURL: function(url, elm, save, name) {
            var ed = tinymce.EditorManager.activeEditor, s = this.settings, base = s.document_base_url;
