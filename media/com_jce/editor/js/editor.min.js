...
!function() {
    ...
    var JoomlaEditor = {
        ...
        init: function(settings) {
            var params, base = (settings = settings || function(key) {
                var settings = Joomla.getOptions ? Joomla.getOptions("plg_editor_jce", {}) : Joomla.optionsStorage.plg_editor_jce || {};
                if (settings && settings[key]) return settings = settings[key], 
                settings = Array.isArray(settings) ? settings[0] : settings;
            }("editor")).base_url, site = this.getSite(base), indent = (/https:\/\//.test(document.location.href) && (base = base.replace(/http:/, "https:")), 
