...
!function() {
    ...
    tinymce.create("tinymce.plugins.CharacterMap", {
        init: function(ed, url) {
            ...
            var html = '<div role="presentation" class="mceCharacterMap mceModalRow">\t<div id="' + (this.editor = ed).id + '_charmapView" role="group"></div>\t<div class="mceCharacterMapDescription">\t\t<h1 id="' + ed.id + '_charmapCodeV"></h1>\t\t<h4 id="' + ed.id + '_charmapCodeN"></h4>\t\t<h3 id="' + ed.id + '_charmapCodeA"></h3>\t\t<h3 id="' + ed.id + '_charmapCodeB"></h3>\t</div></div>';
        ...
        createControl: function(name, cm) {
            var btn, editor = this.editor;
            if ("charmap" === name) return editor.getParam("charmap_custom") ? (btn = cm.createSplitButton(name, {
                ...
                each(editor.getParam("charmap_custom", "", "hash"), function(v, k) {
                    ...
                    v = v.replace(/[^a-z0-9]/gi, ""), menu.add({
                        ...
                        onclick: function() {
                            var chr;
                            chr = v, editor.execCommand("mceInsertContent", !1, "&" + chr + ";"), 
                            editor.focus();
                        }
                    });
                });
