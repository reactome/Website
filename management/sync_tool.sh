...
NEO4J_TRANSACTION_DIR="/var/lib/neo4j/data/transactions/${NEO4J_DB_NAME}" # neo4j:adm
EHLD_ICONS_DIR="/usr/local/reactomes/Reactome/production/Icons"
FIGURES_DIR="${STATIC_DIR}/figures"

declare -A dict=(["solr"]=${SOLR_DATA_DIR} ["icons"]=${EHLD_ICONS_DIR} ["figures"]=${FIGURES_DIR})
...
# At this point solr, etc have been copied to /tmp/<service>
...
folders_to_clean_up() {
    ...
    declare -A folders_to_clean=(["download"]=${DOWNLOAD_PREVIOUS} ["exporter"]=${EXPORTER_FOLDER} ["neo4j-transactions"]=${NEO4J_TRANSACTION_DIR})
    for key in "${!folders_to_clean[@]}"; do
    ...
    for key in "${!folders_to_clean_using_find[@]}"; do
    ...
    # tmp
    local delete_in_tmp=("solr" "icons" "figures")
    for key in "${delete_in_tmp[@]}"; do
        DIR="/tmp/${key}"
        echo "Cleaning ${DIR} folder"
        sudo -S rm -rf ${DIR} &>/dev/null
        OUT=$?
...
owners_and_permissions_serv() {
    declare -A dict_perm=(["solr"]="solr:solr")
    for serv in "${!dict_perm[@]}"; do
        DIR=${dict[${serv}]}
        PERM=${dict_perm[${serv}]}
...
analysis() {
    ...
    unlink ${ANALYSIS_SYMLINK}
    ln -s ${ANALYSIS_FULLNAME} ${ANALYSIS_SYMLINK}
    OUT=$?
    if [[ "$OUT" -ne 0 ]]; then
...
update_download_folder() {
    ...
    sudo unlink ${DOWNLOAD_HOME}/current
    ln -s ${DOWNLOAD_DIR} ${DOWNLOAD_HOME}/current
    OUT=$?
    if [[ "$OUT" -ne 0 ]]; then
...
update_neo4j() {
    echo "Loading ${NEO4J_DB_NAME} dump"
    DUMP_FILE="/tmp/${NEO4J_DB_NAME}.dump"
    sudo neo4j-admin load --from ${DUMP_FILE} --database=${NEO4J_DB_NAME} --force
    sudo chown -R neo4j:adm ${NEO4J_GRAPH_DIR}
    sudo rm -f ${DUMP_FILE}
}
