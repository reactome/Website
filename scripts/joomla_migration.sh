...
db_sync () {
    ...
    sshpass -P passphrase -f <(printf '%s\n' ${OSPASSWD}) ssh -i ${PRIVATE_KEY} -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ${OSUSER}@${SERVER} "export MYSQL_PWD=${DBPASSWD} ; mysql -u ${DBUSER} ${DBNAME} -e "${SAVE_ARTICLE_HITS}""
    OUT=$?
    if [ "$OUT" -ne 0 ]; then
