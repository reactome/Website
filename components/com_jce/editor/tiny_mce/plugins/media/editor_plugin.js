...
!function() {
    ...
    function createPreviewNode(editor, node) {
        var name = node.name, msg = (msg = editor.getLang("media.preview_hint", "Click to activate, %s + Click to toggle placeholder")).replace(/%s/g, tinymce.isMac ? "CMD" : "CTRL"), src = (node.attr("autoplay") && (node.attr("data-mce-p-autoplay", node.attr("autoplay")), 
    ...
    tinymce.PluginManager.add("media", function(ed, url) {
        ...
        function objectActivate(ed, e) {
            ...
            "mousedown" === e.type && VK.metaKeyPressed(e) && (e.target = function(editor, node) {
        ...
        ed.onNodeChange.addToTop(function(ed, cm, n, collapsed, o) {
            ...
            ed.onObjectResized.add(function(ed, elm, width, height) {
                ...
                each(ed.dom.select(".mce-object-preview video, .mce-object-preview audio"), function(elm) {
                    elm.pause();
                }), node && "IMG" === node.nodeName && "object" !== node.getAttribute("data-mce-object") && !isNonEditable(node) && "click" === e.type && VK.metaKeyPressed(e) && (e.target = function(editor, node) {
            ...
            isMediaHtml: function(html) {
                return function(ed, html) {
                    return !(!(ed = ed.dom.create("div", {}, html).firstChild) || 1 !== ed.nodeType) && isPreviewMedia(ed.tagName.toLowerCase());
                }(ed, html);
            }
        };
    });
