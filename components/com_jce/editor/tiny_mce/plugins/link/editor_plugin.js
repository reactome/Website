...
!function() {
    ...
    function updateTextContent(elm, text) {
        text = text.replace(/^[\s ]/, "\ufeff"), tinymce.each(elm.childNodes, function(node) {
            3 == node.nodeType && "" != node.nodeValue && ("innerText" in node ? node.innerText = text : node.textContent = text);
        });
    }
